...
# Beszel iOS lifecycle engine — unofficial community port of Beszel for
# jailbroken iOS.
#
# This file is the lifecycle engine (menu, install/update, repair,
# reconfigure, uninstall/purge, persistent CLI, status/diagnostics/doctor and
# service control). It is NOT the documented entry point: root install.sh is
# a small pipe-safe bootstrap that downloads this engine, verifies it against
# a SHA-256 pinned inside that bootstrap, and then executes this file locally
# as manager.sh. Primary invocation (no clone, no Go toolchain, no manual
# downloads needed):
#
#   curl -fsSL https://raw.githubusercontent.com/nghianguyen150612/Beszel-iOS/iOS/install.sh | sudo sh
#
# Provenance rule: manager persistence copies the engine file that is running
# the transaction (the bootstrap's verified temporary manager.sh, the
# installed manager.sh on later CLI runs, or the repository engine on direct
# script runs). It never fetches manager source from the mutable branch, so
# the persisted /var/lib/beszel-ios/manager.sh is byte-for-byte the engine
# that performed the lifecycle operation. Adopting a newer engine means
# re-running the bootstrap above; a normal beszel-ios update refreshes
# Beszel application binaries only and never replaces the manager.
#
...
# Manager persistence is local-only by design: it copies the exact engine
# file that is running this transaction — the bootstrap's verified temporary
# manager.sh, the installed manager.sh during later CLI invocations, the
# repository engine during direct script runs — or an explicit test fixture
# path. It never downloads manager source from the mutable branch: a
# lifecycle transaction must persist the same code that performed it, so
# when no valid local source exists persistence fails instead of fetching
# something else.
stage_manager_source() {
	...
	if [ -n "$_sms_explicit" ]; then
		...
		case "$0" in
			install.sh | */install.sh | install-beszel.sh | */install-beszel.sh | manager.sh | */manager.sh) _sms_candidate="$0" ;;
		esac
	fi
	[ -n "$_sms_candidate" ] || return 1
...
# install_persistent_manager [source-file] — atomically stages the trusted
# local manager and installs the exact managed wrapper. Unknown occupied paths,
